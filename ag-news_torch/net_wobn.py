...
class VDCNN(nn.Module):
    ...
    def __init__(self, n_classes=2, num_embedding=141, embedding_dim=16, depth=9, n_fc_neurons=2048, shortcut=True, h = 0.1):
        ...
        layers.append(BasicConvResBlock(input_dim=64, n_filters=64, kernel_size=3, padding=1, shortcut=shortcut, h= h) )
        ...
        ds = nn.Conv1d(64, 128, kernel_size=1, stride=1, bias=False) 
        layers.append(BasicConvResBlock(input_dim=64, n_filters=128, kernel_size=3, padding=1, shortcut=shortcut,h= h , downsample=ds))
        for _ in range(n_conv_block_128-1):
            layers.append(BasicConvResBlock(input_dim=128, n_filters=128, kernel_size=3, padding=1, shortcut=shortcut,h= h  ))
        ...
        ds = nn.Conv1d(128, 256, kernel_size=1, stride=1, bias=False) 
        layers.append(BasicConvResBlock(input_dim=128, n_filters=256, kernel_size=3, padding=1, shortcut=shortcut, h=h, downsample=ds))
        for _ in range(n_conv_block_256 - 1):
            layers.append(BasicConvResBlock(input_dim=256, n_filters=256, kernel_size=3, padding=1, shortcut=shortcut,h=h ))
        ...
        ds = nn.Conv1d(256, 512, kernel_size=1, stride=1, bias=False) 
        layers.append(BasicConvResBlock(input_dim=256, n_filters=512, kernel_size=3, padding=1, shortcut=shortcut, h=h, downsample=ds))
        for _ in range(n_conv_block_512 - 1):
            layers.append(BasicConvResBlock(input_dim=512, n_filters=512, kernel_size=3, padding=1, shortcut=shortcut,h=h ))
    ...
    def forward(self, x):
        ...
        #print (out.shape, "......after transpose......")
        out = self.layers(out)
